    ...
    <div class="col-sm-12 div-card">
        <div class="card card-dashboard">
            <div class="card-body">
                ...
                <div class="col-md-12">
                    {!! Form::model($lote, ['route'=>['pagar-conta.update', $lote->id], 'method'=>'put', 'enctype'=> 'multipart/form-data']) !!}
                        {!! Form::hidden('id_contas', 'id[]', ['id' => 'id_contas']) !!}
                            <a href="{{ route('pagar-conta.index') }}" class="btn btn-link">Voltar</a>
                            <button type="submit" style="float:right" class="btn btn-primary check">Salvar</button>
                    {!! Form::close() !!}
                </div>
            </div>
        </div>
