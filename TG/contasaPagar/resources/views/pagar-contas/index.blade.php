                    ...
                    <tbody>
                        <tr>
                            @foreach ($lotes as $lote)
                                ...
                                <td>{{ $lote->dt_alteracao == null ? '-' : date('d/m/Y', strtotime($lote->dt_alteracao)) }}</td>
                                <td>{{ $lote->usuario->name }}</td>
                                <td>{{ $lote->status == 'E' ? 'Em processamento' : $lote->status == 'A' ? 'À processar' : 'Finalizado'  }}</td>
                                <td>{{ $lote->valor_lote }}</td>
                                <td>{{ $lote->dt_transmissao == null ? '-' : date('d/m/Y', strtotime($lote->dt_transmissao)) }}</td>
                                <td class="action-icons">
                                    <a href="{{ route('pagar-conta.processar', ['id'=>$lote->id]) }}">
                                        <i class="fas fa-check" data-toggle="tooltip" data-placement="top"
                                            title="Processar"></i>
                                    </a>
                                    <a href="{{ route('pagar-conta.destroy', ['id'=>$lote->id]) }}" onclick="return confirm('Deseja excluir o registro ?')">
                    ...
                    <tbody>
                        <tr>
                            @foreach ($retorno_lotes as $retorno_lote)
                                ...
                                <td>{{ $retorno_lote->lote->dt_alteracao == null ? '-' : date('d/m/Y', strtotime($retorno_lote->lote->dt_alteracao)) }}</td>
                                <td>{{ $retorno_lote->lote->usuario->name }}</td>
                                <td>{{ $retorno_lote->lote->valor_lote }}</td>
                                <td>{{ $retorno_lote->lote->dt_transmissao == null ? '-' : date('d/m/Y', strtotime($retorno_lote->lote->dt_transmissao)) }}</td>
                                <td>{{ $retorno_lote->status == 'S' ? 'Processada com sucesso' : 'Falha no processamento' }}</td>
